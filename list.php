  	...
  	<div class="row">
		...
		<div class="form-group">
			...
			<div class="form-submit">
				...
				<button type="button" id="delete_cookie" class="btn btn-danger">x</button>    
			</div>
			<div class="form-submit">
			...
			<table class="table table-striped" id="list-table">
				<thead>
					<tr>
						...
						<th>Ghi chú</th>
						<th>Thời gian gửi</th>
						<?php if($auth) echo '<th colspan="2"></th>'; ?>
					</tr>
				</thead>
				<tbody>
				</tbody>
...
<div class="modal fade" id="confirmModal" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="confirmModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		...
		<div class="modal-body">
			<p>Bạn muốn xóa không?</p>
		</div>
		<div class="modal-footer">
			<button type="button" class="btn btn-primary" id="confirmed_delete">OK</button>
		</div>
		</div>
	</div>
</div>
  	...
  	$( "#date_from, #date_to" ).datepicker({ dateFormat: 'dd/mm/yy'}).on("change", function() {
		...
		success   : function(response) {
			if (response) { //If fails
				...
				if(auth) {
					text += '<tr data-id="'+ val.id +'" data-date="'+ val.date +'"><td>' + val.name + '</td><td>' + val.reason + '</td><td>' + val.note + '</td><td>' + val.send_date + '</td><td class="col-action"><button id="edit" type="button" data-toggle="modal" data-target="#staticBackdrop" class="btn btn-primary">Sửa</button></td><td class="col-action"><button type="button" id="delete" class="btn btn-danger" data-toggle="modal" data-target="#confirmModal">Xóa</button></tr>';
				} else {
					text += '<tr data-id="'+ val.id +'" data-date="'+ val.date +'"><td>' + val.name + '</td><td>' + val.reason + '</td><td>' + val.note + '</td><td>' + val.send_date + '</td></tr>';
				}
...
$('#confirmed_delete').attr('data-id', id);
$('#confirmed_delete').attr('data-date', date);
/*
let self = $(this);
$.ajax({
      type: "POST",
...
// Confirm delete
$('body').on('click', '#confirmed_delete', function(){
...
// Delete cookie
$('body').on('click', '#delete_cookie', function(){
	document.cookie = "member_id= ; expires = Thu, 01 Jan 1970 00:00:00 GMT";
	document.cookie = "member_name= ; expires = Thu, 01 Jan 1970 00:00:00 GMT";
	$("#name").val('');
	filter('');
})
...
function filter(name) {
  ...
  $(".no-data").remove();
  if($('#list-table tbody tr:visible').length == 0 && auth) {
    $('#list-table tbody').append('<tr class="no-data"><td colspan="6" style="text-align:center; background-color: rgba(0,0,0,.05);">Không có dữ liệu</td></tr>');
  }
  if($('#list-table tbody tr:visible').length == 0 && !auth) {
